#ifndef LIGHTGBM_METRIC_TOPAVG_METRIC_HPP_
#define LIGHTGBM_METRIC_TOPAVG_METRIC_HPP_
...
class TopavgMetric:public Metric {
 public:
  explicit TopavgMetric(const Config& config) {
    // get eval position
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    for (auto k : eval_at_) {
      name_.emplace_back(std::string("topavg@") + std::to_string(k));
    }
    num_data_ = num_data;
    // get label
  ...
  void CalAvgAtK(std::vector<int> ks, const label_t* label,
    ...
    for (size_t i = 0; i < ks.size(); ++i) {
      bool is_reverse = ks[i] < 0;
      int abs = std::abs(ks[i]);
      data_size_t cur_k = static_cast<data_size_t>(abs);
      if (cur_k > num_data) { cur_k = num_data; }
      for (data_size_t j = cur_left; j < cur_k; ++j) {
        data_size_t rank_idx = j;
        if (is_reverse) {
          rank_idx = num_data - j - 1;
        }
        data_size_t idx = sorted_idx[rank_idx];
        sum_label += label[idx];
      }
      (*out)[i] = sum_label / abs;
      cur_left = cur_k;
  ...
  std::vector<double> Eval(const double* score, const ObjectiveFunction*) const override {
    ...
    if (query_weights_ == nullptr) {
      ...
      for (data_size_t i = 0; i < num_queries_; ++i) {
        ...
        CalAvgAtK(eval_at_, label_ + query_boundaries_[i],
